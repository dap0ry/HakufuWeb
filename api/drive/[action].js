// /api/drive/status + /token + /disconnect + /link-start in one function.
// (link-start replaces the earlier /connect/start path — one segment instead
// of two, so it fits this single-dynamic-segment file alongside the others.)
const crypto = require('crypto');
const { sql } = require('../../lib/db');
const { getCurrentUser } = require('../../lib/auth');
const { applyCors } = require('../../lib/cors');
const { refreshAccessToken, revokeToken } = require('../../lib/google');
...
async function linkStart(req, res) {
  ...
  return res.status(200).json({ link_url: `${base}/api/auth/google/start?state=${encodeURIComponent(code)}` });
}
